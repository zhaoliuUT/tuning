...
class TuningCurveOptimizer_BN:
    """TuningCurveOptimizer_BN Class (1 or 2 populations)
    # Binary distribution
    # Important Note: FP, FM must be in (0,1).
    Attributes:
    ...
    Methods:
        __init__(self,
                 TuningCurve_init, # initial input (TuningCurve_BN object)
                 fp, # upper bound
                 fm, # lower bound
                 NUM_THREADS = 8,
                 MC_ITER_INFO = 1e5,
    ...
    Static Methods:
        load_res_list(filename) # Usage: TuningCurveOptimizer_BN.load_res_list(filename)
    """
    def __init__(self,
        ...
        if np.any(fp > 1.0) or np.any(fp < 0) or np.any(fm < 0) or np.any(fm > fp):
    ...
    def iterate(self, NUM_ITER = 1, INTER_STEPS = 1, PRINT = True, FILE_NAME = "", ADD_TIME = True, ftol = 1e-15, disp = False):
        ...
        def opt_fun(x, self):
            ...
            return bn_mc_mean_grad_red_pop(MIgrad, self.numCent, self.delta,\
        ...
        def grad_fun(x, self):   
            ...
            x_mean = bn_mc_mean_grad_red_pop(x_grad, self.numCent, self.delta, \
                                          x.reshape((self.numPop, self.numBin)), self.stim, self.tau, \
    ...
    def plot_res_id(self, i, ALL = True):
        ...
        tuning_curve = TuningCurve_BN(self.res_list['x'][i], self.stim, self.nu,\
                                   self.tau, self.delta, self.res_list['obj'][i], self.res_list['grad'][i])
        tuning_curve.plot(ALL)
        
        
    def plot_animation(self, FILE_NAME = "", ADD_TIME = True, XTICKS_IDX_LIST = [], interval = 1000, color = False, dt = 1):
        tc_list = []
        for i in range(self.res_len):
            tc = TuningCurve_BN(self.res_list['x'][i], self.stim, self.nu, self.tau, self.delta, \
        ...
        TuningCurve_BN.animation_tc_list(tc_list, FILE_NAME, ADD_TIME, FP = self.fp, FM = self.fm, \
    ...
    def load_res_list(filename):
        ...
        tc = TuningCurve_BN(res_list['x'][0], res_list['stim'], res_list['nu'], res_list['tau'], \
                         res_list['delta'], res_list['obj'][0], res_list['grad'][0])
        tc_opt = TuningCurveOptimizer_BN(tc, res_list['fp'], res_list['fm'], res_list['NUM_THREADS'],\
